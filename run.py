...
if __name__ == "__main__":
    ...
    # feature build
    parser.add_argument(
        "--time2text", action="store_true", help="time2text", default=False
    )

    args = parser.parse_args()
    args.use_gpu = True if torch.cuda.is_available() and args.use_gpu else False
    if args.root_path == "./dataset/ETT-small/":
        args.root_path = (
            os.path.join(args.root_path, args.data_path.split(".")[0]) + "/"
        )
    ...
    if args.task_name == "long_term_forecast":
        ...
        args.root_path = os.path.join(args.root_path, args.seasonal_patterns) + "/"
